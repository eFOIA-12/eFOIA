...
public abstract class ExploreByTouchHelper extends AccessibilityDelegateCompat {
   ...
   private AccessibilityNodeInfoCompat createNodeForChild(int var1) {
      ...
      var3.setEnabled(true);
      var3.setClassName(DEFAULT_CLASS_NAME);
      this.onPopulateNodeForVirtualView(var1, var3);
      if(var3.getText() == null && var3.getContentDescription() == null) {
         throw new RuntimeException("Callbacks must add text or a content description in populateNodeForVirtualViewId()");
      } else {
         var3.getBoundsInParent(this.mTempParentRect);
         if(this.mTempParentRect.isEmpty()) {
            throw new RuntimeException("Callbacks must set parent bounds in populateNodeForVirtualViewId()");
         } else {
            int var2 = var3.getActions();
            if((var2 & 64) != 0) {
               ...
               var3.setPackageName(this.mView.getContext().getPackageName());
               var3.setSource(this.mView, var1);
               var3.setParent(this.mView);
               if(this.mFocusedVirtualViewId == var1) {
                  var3.setAccessibilityFocused(true);
                  var3.addAction(128);
               } else {
                  var3.setAccessibilityFocused(false);
                  var3.addAction(64);
               }

               if(this.intersectVisibleToUser(this.mTempParentRect)) {
                  var3.setVisibleToUser(true);
                  var3.setBoundsInParent(this.mTempParentRect);
               }

               this.mView.getLocationOnScreen(this.mTempGlobalRect);
               var1 = this.mTempGlobalRect[0];
               var2 = this.mTempGlobalRect[1];
               this.mTempScreenRect.set(this.mTempParentRect);
               this.mTempScreenRect.offset(var1, var2);
               var3.setBoundsInScreen(this.mTempScreenRect);
               return var3;
            }
         }
      }
   ...
   private AccessibilityNodeInfoCompat createNodeForHost() {
      ...
      while(var4.hasNext()) {
         ...
         var1.addChild(this.mView, var3.intValue());
      }
   ...
   public AccessibilityNodeProviderCompat getAccessibilityNodeProvider(View var1) {
      if(this.mNodeProvider == null) {
         this.mNodeProvider = new ExploreByTouchHelper.ExploreByTouchNodeProvider(null);
      }
